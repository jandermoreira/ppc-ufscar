...
\NewDocumentCommand{\Alerta}{ m }{\noindent\textcolor{red!80!black}{\faExclamationTriangle}~\textit{#1}\bigskip\par}
...
\PDPrintChanges<Histórico de versões>
...
\begin{table}[H]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{0.25\linewidth}>{\RaggedRight\arraybackslash}p{0.65\linewidth}}
        ...
        \\
        Matriz curricular &
        A definição da matriz corresponde à organização das atividades curriculares no curso. Permite especificar as atividades curriculares por área, além de indicar a carga de optativas e listar possíveis eletivas. Usando \Macro{PPCDefinaMatrizCurricular}, são listadas as atividades curriculares período por período e indicando quais são obrigatórias e quais são optativas. Essa definição permite saber o número de períodos, quais são as atividades curriculares do curso e seu caráter, quais são as áreas, determinar a carga horária total e por período. A partir dela é gerada o elemento gráfico grade curricular e as listagens de atividades curriculares por período. \\
    \end{tabular}
\end{table}
...
\subsection{Definição de atividades curriculares}\label{sec:definicao-de-disciplina}

Cada disciplina deve ser especificada usando-se \Macro{PPCDefinaAtividade}. O primeiro parâmetro é o identificador da disciplina, usualmente um apelido comum, como \PDInline{tcc} para \textit{Trabalho de Conclusão de Curso}, por exemplo.

\begin{Macrodef}{PPCDefinaAtividade}{\MArg{id-disciplina}\MArg{lista-atributos}}{}
...
\begin{PDExample}
    ...
    A disciplina \PPCAtividade{cálculo1}{nome} possui carga horária total de \PPCAtividade{cálculo1}{horas total}\,h.
\end{PDExample}

Alguns atributos merecem destaque, sendo apresentados na \Cref{tab:atributos-atividades}.

\begin{table}
    \caption{Atributos de atividades curriculares que merecem destaque.}
    \label{tab:atributos-atividades}
    \centering
    \begin{tabular}{>{\raggedright\arraybackslash}p{0.35\linewidth}>{\RaggedRight\arraybackslash}p{0.55\linewidth}}
        ...
        Esse atributo é opcional e pode ser omitido. Quando existir, ele é usado no nome da disciplina na apresentação gráfica da matriz curricular (\MacroRef{PPCMatrizCurricular}). Assim, nomes mais longos de atividades curriculares podem ter uma versão mais curta para que caibam nos espaços reservados. Siglas significativas também podem ser usadas, como TCC.
        \\
...
A matriz curricular, feita com \MacroRef{PPCDefinaMatrizCurricular}, é a forma de organizar o conjunto de atividades curriculares.
...
O texto ``trimestre'' é usado na apresentação das tabelas de atividades curriculares por trimestre apenas.

\subsubsection{Áreas e Eixos}
\index{área}
\index{eixo}
O curso é organizado em quatro grandes \textbf{áreas}: Terra, Fogo, Água e Ar.

Para a área da água, seu identificador é \PDInline{hidro} e seu \PDInline{nome} é ``Ciências da Água''. Essa área não possui atividades curriculares nos trimestres 1, 4, 6 ou 8, mas tem duas atividades curriculares no segundo trimestre, uma no terceiro, três no quinto e uma no sétimo.
...
No lugar de \PDInline{área}, o termo \PDInline{eixo} também pode ser usado, sendo apenas um \textit{alias} (ver \MacroRef{PPCArea} e \MacroRef{PPCEixo}).

\index{optativas}
\index{atividades curriculares!optativas}
\index{obrigatórias}
\index{atividades curriculares!obrigatórias}
A especificação da lista de atividades curriculares é feita pelos identificadores usados na definição da disciplina. Eles podem ser prefixados para modificar como a disciplina é interpretada, como descrito na \Cref{tab:prefixos-atividades}.

\begin{table}[H]
    \caption{Prefixos usados nas listas de atividades curriculares.}
    \label{tab:prefixos-atividades}
    \centering
...
\Alerta{Não deve haver espaços entre o prefixo e o identificador da disciplina.}

\bigskip
No exemplo, as atividades curriculares \PDInline{crio-fund} e \PDInline{hidro-geo} são marcadas como optativas, o que é indicado pelo asterisco que precede o identificador. Atividades sem o asterisco são consideradas obrigatórias. Sempre que uma disciplina é incluída na matriz, seu atributo \PDInline{caráter} é ajustado para \PDInline{Obrigatório} ou \PDInline{Optativo}.

\subsubsection{Carga de optativas}\label{sec:carga-de-optativas}
Em um determinado período do curso, várias optativas podem ser oferecidas. No exemplo dado, são ofertadas cinco atividades curriculares optativas no 6º trimestre. Porém, é esperado que o aluno cumpra três delas para integralizar sua carga horária.
...
A contabilização da carga horária do período é a soma das obrigatórias com a soma das cargas das optativas listadas. (Ver \MacroRef{PPCCargaHoraria}.)
...
Para essa situação, o identificador da disciplina pode ser precedido de um sinal de menos (\PDInline{-}), o que faz com que a disciplina seja incluída na grade como obrigatória, mas desconsiderando sua carga horária. Para dar suporte para compensar uma determinada quantidade de horas, ela pode ser incluída na lista do semestre na forma \PDInline{+}$n$ (e.g, \PDInline{+120}) para acrescentar manualmente $n$~horas à carga horária.

No exemplo, \PDInline{estagio} e \PDInline{tcc} seriam atividades curriculares com 360\,h de carga horária cada uma. A ``área'' de estágio e TCC as inclui como obrigatórias, sem contabilizar as horas de nenhuma delas. Manualmente são incluídas 360\,h no período.\footnote{Naturalmente, \PDInline{8 = {-estagio, tcc}} teria o mesmo efeito final.}
...
\index{eletivas}
\index{atividades curriculares!eletivas}
Caso seja necessário, uma lista com atividades curriculares eletivas selecionadas pode ser criada.

\begin{PDListing}
    eletivas = {geo-hist, astro-elem},
\end{PDListing}

Essa listagem atualiza a contagem de horas de eletivas (ver \MacroRef{PPCCargaHoraria}).
...
\begin{Macrodef}{PPCAtividade}{\MArg{id-disciplina}\MArg{atributo}}{}
    Resulta no \Argument{atributo} da disciplina \Argument{id-disciplina} especificado com \MacroRef{PPCDefinaAtividade}.
\end{Macrodef}
...
\begin{Macrodef}{PPCCargaHoraria}{\OArg{nome-matriz}\MArg{nome-do-item}\OArg{texto-adicional}}{}
    ...
    \begin{tabular}{l>{\raggedright\arraybackslash}p{0.7\linewidth}}
        \hfill\textbf{Nome}\hfill\null & \hfill\textbf{Descrição}\hfill\null                                         \\
        \cmidrule(lr){1-1} \cmidrule(lr){2-2} \\
        \texttt{total}                 & Número de horas total do curso.                                             \\
        \texttt{obrigatória}           & Número de horas total de atividades curriculares obrigatórias.              \\
        \texttt{obrigatória $n$}       & Número de horas de atividades curriculares obrigatórias para o período $n$. \\
        \texttt{opcional}              & Número de horas total de atividades curriculares opcionais.                 \\
        \texttt{opcional $n$}          & Número de horas de atividades curriculares opcionais para o período $n$.    \\
        \texttt{eletiva}               & Número total de atividades curriculares definidas como eletivas.            \\
        \texttt{teórica}               & Número de horas teóricas total.                                             \\
        \texttt{prática}               & Número de horas práticas total.                                             \\
        \texttt{extensionista}         & Número de horas extensionistas total.                                       \\
        \texttt{estágio}               & Número de horas de estágio total.                                           \\
    \end{tabular}
...
\begin{Macrodef}{PPCEixo}{\OArg{nome-matriz}\MArg{id-do-eixo}\MArg{atributo}}{}
    \Macro{PPCEixo} recupera uma informação de eixo \Argument{id-do-eixo} definida em uma matriz curricular (atualmente, apenas \PDInline{nome} está disponível).
...
\Alerta{\textbf{Importante:} A modificação do caráter de uma disciplina (de optativa para obrigatória ou vice-versa) em uma segunda matriz, implica na alteração permanente da configuração da disciplina\footnote{O uso de grupos do \LaTeX\ não foram testados e podem, em princípio, evitar essa modificação.}. Em quaisquer listagens ou gráficos, o novo caráter será usado, independentemente da matriz escolhida.}
...
\subsection{O pacote \PDInline{attrtoolbox}}
...
\index{listas disponíveis}
Várias listas são criadas com os dados fornecidos e estão apresentadas na \Cref{tab:listas}

\begin{table}[H]
    \caption{Listas disponíveis para o documento.}
    \label{tab:listas}
    \centering
    \begin{tabular}{>{\raggedright\arraybackslash\ttfamily}l>{\raggedright\arraybackslash}p{0.55\linewidth}}
        \hfill\normalfont\textbf{Lista}\hfill\null & \hfill\textbf{Descrição}\hfill\null \\
        \cmidrule(lr){1-1} \cmidrule(lr){2-2} \\
        matriz-principal-áreas &
        Relação de todos os \textit{id} de área da matriz curricular.
        \\
        \Argument{nome-matriz}-áreas &
        Relação de todos os \textit{id} de área da matriz curricular nomeada \Argument{nome-matriz}.
        \\
        matriz-principal-áreas-$n$ &
        Relação das atividades os \textit{id} de área da matriz curricular.
        \\
        \Argument{nome-matriz}-áreas-$n$ &
        Relação de todos os \textit{id} de área da matriz curricular nomeada \Argument{nome-matriz} no período $n$.
        \\
        matriz-principal-atividades &
        Relação de todas as atividades curriculares inseridas na matriz curricular.
        \\
        \Argument{nome-matriz}-atividades &
        Relação de todas as atividades curriculares inseridas na matriz curricular nomeada \Argument{nome-matriz}.
        \\
        matriz-principal-eletivas &
        Relação de todas as atividades curriculares eletivas inseridas na matriz curricular.
        \\
        \Argument{nome-matriz}-eletivas &
        Relação de todas as atividades curriculares eletivas inseridas na matriz curricular nomeada \Argument{nome-matriz}.
        \\
        matriz-principal-período-$n$ &
        Relação das atividades curriculares registradas no período $n$.
        \\
        \Argument{nome-matriz}-período-$n$ &
        Relação das atividades curriculares do período $n$ da matriz curricular nomeada \Argument{nome-matriz}.
        \\
        \Argument{id-disciplina}-pré-requisitos &
        Relação dos pré-requisitos definidos para a disciplina \Argument{id-disciplina}.
        \\
    \end{tabular}
\end{table}

\begin{PDListing}
    % Apresentação de todas as fichas de atividades curriculares da matriz
    \ATListForEach{matriz-principal-atividades}{\identificador}{
        \PPCFichaAtividade{\identificador}
    }
\end{PDListing}

\begin{Macrodef}{PPCOrdeneAtividades}{\MArg{nome-lista}\MArg{nome-atributo}}{}
    Esta macro permite a ordenação (ordem alfabética) de uma lista de atividades curriculares \Argument{nome-lista} usando como critério o atributo \Argument{nome-atributo}.
\end{Macrodef}

\begin{PDListing}
    % Ordena as disciplinas da matriz curricular por nome
    \PPCOrdeneAtividades{matriz-curricular-atividades}{nome}
\end{PDListing}


\section{Informações padronizadas}
Uma série de comandos foi disponibilizada no intuito de facilitar a edição e apresentação do texto completo.
...
\begin{Macrodef}{PPCFichaAtividade}{\MArg{id-disciplina}}{}
    \Macro{PPCFichaAtividade} gera a ficha descritiva padrão de uma disciplina identificada por \Argument{id-disciplina}.
\end{Macrodef}
...
\begin{Macrodef}{PPCQuadroPeriodo}{\OArg{nome-matriz}\MArg{períodos}\OArg{legenda}}{}
    ...
    O quadro padrão é formado por uma tabela com as atividades curriculares do período.
...
\begin{Optiondef}{separação}{\Argument{comprimento}}{Inicialmente: \texttt{0.2cm}}
    A separação indica o espaço entre as atividades curriculares e as bordas. Esse valor é uma referência, pois a figura como um todo será redimensionada.
\end{Optiondef}

\begin{Optiondef}{cor fundo obrigatória}{\Argument{cor}}{Inicialmente: \texttt{blue!40}}
    Determina a cor de fundo das caixas das atividades curriculares obrigatórias.
\end{Optiondef}

\begin{Optiondef}{cor texto obrigatória}{\Argument{cor}}{Inicialmente: \texttt{black}}
    Determina a cor do texto nas caixas das atividades curriculares obrigatórias.
\end{Optiondef}

\begin{Optiondef}{cor horas obrigatória}{\Argument{cor}}{Inicialmente: \texttt{black}}
    Determina a cor do texto usado para a carga horária nas caixas das atividades curriculares obrigatórias.
\end{Optiondef}

\begin{Optiondef}{cor gradiente obrigatória}{\Argument{cor}}{Inicialmente: \texttt{blue!10}}
    Os espaços para as horas dentro das caixas das atividades curriculares possuem um fundo degradê. Esse gradiente parte da cor de fundo da caixa e termina na cor especificada para \Option{cor gradiente obrigatória}.
\end{Optiondef}

\begin{Optiondef}{cor fundo optativa}{\Argument{cor}}{Inicialmente: \texttt{teal!40}}
    Determina a cor de fundo das caixas das atividades curriculares optativas.
\end{Optiondef}

\begin{Optiondef}{cor texto optativa}{\Argument{cor}}{Inicialmente: \texttt{black}}
    Determina a cor do texto nas caixas das atividades curriculares optativas.
\end{Optiondef}

\begin{Optiondef}{cor horas optativa}{\Argument{cor}}{Inicialmente: \texttt{black}}
    Determina a cor do texto usado para a carga horária nas caixas das atividades curriculares optativas.
\end{Optiondef}

\begin{Optiondef}{cor gradiente optativa}{\Argument{cor}}{Inicialmente: \texttt{teal!10}}
    Os espaços para as horas dentro das caixas das atividades curriculares possuem um fundo degradê. Esse gradiente parte da cor de fundo da caixa e termina na cor especificada para \Option{cor gradiente optativa}.
\end{Optiondef}

\begin{Optiondef}{cor fundo área}{\Argument{cor}}{Inicialmente: \texttt{black!10}}
    As atividades curriculares de cada área ficam contidas em uma caixa que engloba todos os períodos. Esta é a cor de fundo desta caixa. Ela também é usada como cor de fundo para o nome das áreas.
\end{Optiondef}
...
\begin{Optiondef}{visão do aluno}{\PDInline{true}|\PDInline{false}}{Inicialmente: \PDInline{false}}
    Na apresentação regular da matriz curricular, todas as atividades curriculares optativas disponíveis para o aluno são apresentadas, mesmo que ele tenha que cumprir uma quantidade menor delas.
...
\begin{itemize}
    \item Substituição do texto usado nas bibliografias das atividades curriculares por uma lista de rótulos \BibTeX\ para formatação automática segundo a ABNT.
    \item Geração de grade de atividades curriculares com conexões representando pré-requisitos.
...
Há um exemplo de projeto pedagógico de curso disponível no \href{https://github.com/jandermoreira/ppc-ufscar/tree/master/exemplo}{Github}. Vários usos dos recursos deste pacote podem ser consultados.
